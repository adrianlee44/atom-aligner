...
module.exports = {
  ...
  formatRange: function (editor, range, character, offsets, sectionizedLines) {
    ...
    range.getRows().forEach(function (currentRow) {
      let currentLine = '';
      const tokenizedLine = helper.getTokenizedLineForBufferRow(
        editor,
        currentRow
      );
      const lineCharacter = helper.getAlignCharacter(
        editor,
        currentRow,
        character,
        config
      ...
      sectionizedLine.sections.forEach(function (section, index) {
        ...
        if (config.multiple) {
          ...
          alignment =
            (config.multiple[type] && config.multiple[type].alignment) ||
            'left';
        } else {
    ...
    if (atom.config.get('aligner.alignComments')) {
      lines.forEach(function (line, index) {
        ...
        if (sectionizedLine && sectionizedLine.trailingComment) {
          lines[index] +=
            this.buildWhitespaces(maxLength - line.length) +
