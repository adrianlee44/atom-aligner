 ...
 */
function getAlignCharacter (editor, row) {
  ...
  for (let i = 0; i < tokenized.tokens.length; i++) {
    ...
    let tokenValue = token.value.trim();
    ...
    for (let j = 0; j < token.scopes.length; j++) {
      if (token.scopes[j].match(config.scope) != null) {
        return tokenValue;
      }
    }
  }
  ...
  while (start > -1 || end < total) {
    if (start > -1) {
      ...
      if (startLine && editor.indentationForBufferRow(start) == indent) {
        ...
        } else if ((sectionizedLine = parseTokenizedLine(startLine, character, config)) && sectionizedLine.isValid()) {
    ...
    if (end < total + 1) {
      ...
      if (endLine && editor.indentationForBufferRow(end) == indent) {
        ...
        } else if ((sectionizedLine = parseTokenizedLine(endLine, character, config)) && sectionizedLine.isValid()) {
