  ...
  for (let i = 0; i < tokenized.tokens.length; i++) {
    ...
    for (let j = 0; j < token.scopes.length; j++) {
      ...
      if (
        doesScopeMatch &&
        (!character ||
          operatorConfig.canAlignWith(character, tokenValue, characterConfig))
      ) {
        return tokenValue;
      }
    }
...
function parseTokenizedLine(tokenizedLine, character, config) {
  ...
  if (
    !isTokenizedLineCommentOnly(tokenizedLine) &&
    atom.config.get('aligner.alignComments')
  ) {
    ...
    for (let index = tokenizedLine.tokens.length - 1; index >= 0; index--) {
      ...
      if (token.matchesScopeSelector('comment')) {
        sectionizedLine.trailingComment =
          _formatTokenValue(token, tokenizedLine.invisibles) +
          sectionizedLine.trailingComment;
      } else {
  ...
  for (let index = 0; index < tokenizedLine.tokens.length; index++) {
    ...
    let doesScopeMatch = token.scopes.some(
      (scope) => !!scope.match(config.scope)
    );

    if (
      doesScopeMatch &&
      operatorConfig.canAlignWith(
        character,
        _sanitizeTokenValue(tokenValue),
        config
      ) &&
      (!afterCharacter || config.multiple)
    ) {
      sectionizedLine.prefix = operatorConfig.isPrefixed(
        _sanitizeTokenValue(tokenValue),
        config
      );
  ...
  while (start > -1 || end < total) {
    if (start > -1) {
      ...
      if (startLine && editor.indentationForBufferRow(start) == indent) {
        ...
        if (isTokenizedLineCommentOnly(startLine)) {
    ...
    if (end < total + 1) {
      ...
      if (endLine && editor.indentationForBufferRow(end) == indent) {
        ...
        if (isTokenizedLineCommentOnly(endLine)) {
...
function getTokenizedLineForBufferRow(editor, row) {
  ...
  let tokenizedBuffer =
    editor.tokenizedBuffer || editor.displayBuffer.tokenizedBuffer;
 ...
 * @return {boolean}
 */
function isTokenizedLineCommentOnly(tokenizedLine) {
  // For some reason `isComment` on the tokenized line does not mean
  // the full line is comment. It is true if there is at least one comment token
  if (!tokenizedLine.isComment()) return false;

  return tokenizedLine.tokens.every((token) => {
    // Token is either comment or whitespace
    return token.matchesScopeSelector('comment') || !token.value.trim();
  });
}
