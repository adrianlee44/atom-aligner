...
describe('Aligner', () => {
  ...
  beforeEach(() => {
    ...
    waitsForPromise(() => {
      return atom.packages.activatePackage(
        path.join(__dirname, 'fixtures/packages/aligner-coffeescript')
      );
    });
  ...
  it('should align multiple items correctly', () => {
    ...
    atom.commands.dispatch(editorView, 'aligner:align');
    expect(editor.lineTextForBufferRow(21)).toBe(
  ...
  it('should not align comments', () => {
    ...
    atom.commands.dispatch(editorView, 'aligner:align');
    expect(editor.lineTextForBufferRow(42)).toBe(
      "  comment1: 'something' # first comment"
    );
    expect(editor.lineTextForBufferRow(43)).toBe(
      "  comment2: 'hi' # second comment"
    );
    expect(editor.lineTextForBufferRow(44)).toBe(
      '  # this is a comment only line'
    );
    expect(editor.lineTextForBufferRow(45)).toBe(
...
describe('Aligning javascript', () => {
  ...
  it('should not align comments', () => {
    ...
    expect(editor.lineTextForBufferRow(14)).toBe(
  ...
  it('should align trailing comment with or without operator in range', function () {
    ...
    atom.commands.dispatch(editorView, 'aligner:align');
    expect(editor.lineTextForBufferRow(16)).toBe(
